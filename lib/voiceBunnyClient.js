...
var VoiceBunny = module.exports = {
    ...
    getProjects: function(status, page, itemsPerPage, includeTests, callback) {
        ...
        console.log('getProjects postParams: ' + JSON.stringify(postParams));
    
        this.execAuthRequest(uri, postParams, callback, "GET", 200);   
    },
    ...
    rejectRead: function(readId, rejectComments, callback) {
        var uri = '/reads/reject/' + readId + '.json';
        var postParams = {
            rejectComments: rejectComments
        };        
        console.log('reject postParams: ' + JSON.stringify(postParams));  
        this.execAuthRequest(uri, postParams, callback, "POST", 200); 
    },
    ...
    languages: function(callback) {
        var u = "https://" + this.baseUrl + isProd + "/languages.json";
        console.log(u);
        this.execRequest(u, callback);   
    },

    genderAndAges: function(callback) {
        var u = "https://" + this.baseUrl + isProd + "/genderAndAges.json";
        console.log(u);
        this.execRequest(u, callback);   
    },
        ...
        var req = https.request(options, function(res) {
            ...
            res.on('data', function(data) {
                if (res.statusCode == expectedResponse) {
                    ...
                    var errMsg = data || '';
                    var e = new Error("Failed to execute " + method + ": " + res.statusCode + " error: " + errMsg + ". Check http://voicebunny.com/developers for status code definitions");
                    if (callback) {
                        callback(e, data);            
                    }
                    else {
